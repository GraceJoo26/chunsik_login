...
const chunsik = document.querySelector('#chunsik');
...
function reset(event) {
   ...
   chunsik.classList.remove('playing');

    clearTimeout(timer);
    timer = setTimeout( () => {
       chunsik.classList.remove('look-away', 'down', 'up');
        rotate3d(0,0,0,0);
    }, 1 );
}
...
function look(event) {
    ...
    const chunsikRect =chunsik.getBoundingClientRect();
    ...
    const distCaret =chunsikRect.left +chunsikRect.width/2 - inputRect.left - caretPos;
    const distInput =chunsikRect.top +chunsikRect.height/2 - inputRect.top;
...
function lookAway(event) {
   ...
   chunsik.classList.add( 'look-away', distInput < 0 ? 'up' : 'down' );

    clearTimeout(timer);
    timer = setTimeout( () => {
       chunsik.classList.add( 'playing' );
    }, 300);
}
